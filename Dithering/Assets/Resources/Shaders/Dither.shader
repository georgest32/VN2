    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				float2 noiseUV = i.uv * _NoiseTex_TexelSize.xy * _MainTex_TexelSize.z;
...
//                  float thresholdLum = dot(threshold, float3(0.299f, 0.587f, 0.114f));
  //                float rampVal = lum < thresholdLum ? thresholdLum - lum : 1.0f;
	//			    float3 rgb = tex2D(_ColorRampTex, float2(rampVal, 0.5f));
		//		    return float4((2 * rgb + col) * rgb + (rgb * 0.5), 1.0f);
//                }

//dreamy
//                else if (playerMode == 1){
				    float thresholdLum = dot(threshold, float3(0.739f, 0.087f, 0.154f));
                    float rampVal = lum < thresholdLum ? thresholdLum - lum : 1.0f;
				    float3 rgb = tex2D(_ColorRampTex, float2(rampVal, 0.5f));
			        return float4(((6 * rgb - col + rgb) * col), 1.0f);
//                }
    ...
    //kinda cool
                //faceripper (dreamy threshlum)
				//return float4((rgb + col/rgb)*col, 1.0f);
                //nightmare (its a start)
				//return float4((5*rgb - col+rgb)*(col-rgb), 1.0f);
